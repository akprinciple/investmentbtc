		...
		<main>
				...
				<div class="dashboard">
					<div class="box">
							...
							<button class="details-button"><h3>Deposit Balance Wallet</h3></button>
					
						<div class="value">
                            $<?php
                $sel = mysqli_query($connect, "SELECT * FROM wallet WHERE user_id= '{$_SESSION['id']}'");
                ...
                $sq = mysqli_query($connect, "SELECT SUM(amount * 0.05 * (DATEDIFF('$date', date))) AS 
                             total  FROM investment WHERE user_id = '{$_SESSION['id']}' AND DATEDIFF('$date', date) < 31");
               $rw = mysqli_fetch_array($sq);
                   echo number_format($rw['total'],2);
					...
					<div class="box">
                        ...
                        $sq = mysqli_query($connect, "SELECT SUM(amount) AS 
                                     total  FROM transactions WHERE user_id = '{$_SESSION['id']}'&& status='approved'");
                       $rw = mysqli_fetch_array($sq);
                           echo number_format($rw['total'],2);
                     ?>
                        </div>
					...
					<div class="box">
                        ...
                        $sq = mysqli_query($connect, "SELECT SUM(amount) AS 
                                     total  FROM withdrawal WHERE user_id = '{$_SESSION['id']}' && status='approved'");
                       $rw = mysqli_fetch_array($sq);
                           echo number_format($rw['total'],2);
                     ?>
                        </div>
                ...
                <?php 
                    $sel = mysqli_query($connect, "SELECT * FROM login WHERE user_id = '{$login_id}' ORDER BY id DESC LIMIT 1,1");
                    foreach ($sel as $key) {
                ...
                <div class="container">
                    ...
                    <?php
            $sel = mysqli_query($connect, "SELECT * FROM investment WHERE user_id= '{$_SESSION['id']}' ORDER BY id DESC LIMIT 1");
            $n = 1;
